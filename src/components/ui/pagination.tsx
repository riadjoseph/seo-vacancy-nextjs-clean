...
import { PaginationData, generatePageUrl } from '@/utils/pagination'
      ...
      <div className="flex items-center space-x-1">
        {data.ranges.map((range) => {
          ...
          return (
            <Link
              key={range.key}
              href={generatePageUrl(baseUrl, range.page!, searchParams)}
              className={cn(
...
export function PaginationSummary({ data, className }: PaginationSummaryProps) {
  ...
  // Don't show summary if there are no items
  if (totalItems === 0) {
    return (
